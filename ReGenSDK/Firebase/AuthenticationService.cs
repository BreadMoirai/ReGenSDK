    ...
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public Task SendRecoverPasswordEmail(string email)
        {
            ...
            return _auth.SendPasswordResetEmailAsync(email)
                ...
                .Failure<FirebaseException>(e => Debug.LogException(Exception($"Failed to send recovery email to {email}: {e.Message}", e)));
        }
        ...
        public Task<FirebaseUser> SignInUserWithEmail(string email, string password)
        {
            ...
            return _auth.SignInWithEmailAndPasswordAsync(email, password)
                ...
                .Failure<FirebaseUser, FirebaseException>(e => Debug.LogError(Exception($"Failed to log in user {email}", e)));
        }
            ...
            if (_auth.CurrentUser != null)
            {
                return _auth.CurrentUser.LinkWithCredentialAsync(credential)
                    .Success(user => Debug.LogFormat("Credentials successfully linked to Firebase user: {0} ({1})",
                        user.DisplayName, user.UserId))
                    .Failure<FirebaseUser, FirebaseException>(exception => Debug.LogException(Exception($"Failed to link facebook to user {_auth.CurrentUser.UserId}", exception)));
            }
